...
pub async fn run(socket_path: &str, key_manager: Arc<KeyManager>) {
        ...
        match listener.accept().await {
            Ok((mut stream, _)) => {
                ...
                tokio::spawn(async move {
                    if let Err(e) = handle_connection(&mut stream, &km).await {
...
async fn handle_connection(
    ...
    // Process
    let response = process_request(request, km);
...
fn process_request(request: AdminRequest, km: &KeyManager) -> AdminResponse {
    match request {
